...
// ふりがな付きで文字数が増えるぶん、余裕を持たせる。
const MAX_TOKENS = Number(process.env.MAX_TOKENS || 1500 + TOP_N * 350);
...
【ふりがな（ルビ）: 難しい漢字だけに付ける】
title_ja / summary / take / overview の日本語で、【読み間違えやすい・難しい漢字】に
だけ読み仮名を付ける。記法は必ず {漢字|かんじ} の形（波括弧で囲み、縦棒の左に漢字、
右にひらがなの読み）。
- 付ける対象: 難読漢字・専門用語・固有名詞（人名/地名/組織名）・難しい熟語。
  例: {地政学|ちせいがく} {半導体|はんどうたい} {為替|かわせ} {懸念|けねん} {岸田|きしだ}
- 付けない対象: 中学生が普通に読める平易な漢字。
  例: 日本・今日・世界・会社・社会・政府・時間・大きい などには付けない。
- ひらがな・カタカナ・英数字・記号には付けない。読みは正確な現代仮名遣いで。
- 例文: {半導体|はんどうたい}の{輸出|ゆしゅつ}をめぐり、市場は{懸念|けねん}を強めた。
